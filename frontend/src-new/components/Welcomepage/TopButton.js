...
export default function ButtonVariants({ onTripTypeChange }) {
  const [selectedType, setSelectedType] = useState('intra')
  ...
  return (
    <>
      <button className="btn btn-primary" onClick={() => handleButtonClick('intra')}>
        Urban Trip
      </button>
      <button className="btn btn-success" onClick={() => handleButtonClick('inter')}>
        Inter-urban Trips
      </button>
